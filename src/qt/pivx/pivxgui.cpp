...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("CryptoSaga client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("CryptoSaga"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("CryptoSaga"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }

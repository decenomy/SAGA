    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of SAGA earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 SAGA");
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("SAGA")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair SAGA
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(252,188,86));
	set1->setColor(QColor(0,199,115));

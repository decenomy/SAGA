...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SAGA specific functionality (Masternodes) (0-1, default: %u)"), 0));
...
/** Sanity checks
 *  Ensure that CryptoSaga is running in a usable environment with all
...
#endif
    LogPrintf("CryptoSaga version %s (%s)\n", version_string, CLIENT_DATE);
}

/** Initialize cryptosaga.
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. CryptoSaga is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single CryptoSaga process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. CryptoSaga is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //CryptoSaga specific: spork DB's
                ...
                // CryptoSaga: load previous sessions sporks if we have them.
